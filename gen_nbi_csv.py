...
def prepare(append_rows):
    script_file=open(root+'load_nbi_data.script','w')
    post_file=open(root+'post_nbi_data.script','w')
    
    writers={}
    file_mode='a+' if append_rows else 'w'
    #for node definitions
    for mapper in node_mappers:
        f=open(root+'nbi_'+mapper.label+'.csv',file_mode,newline='')
        writer=csv.writer(f)
        header=['id']
        ...
        script_file.write('using periodic commit 500 LOAD CSV WITH HEADERS FROM "file:///nbi_'+mapper.label+'.csv" AS row merge (n:'+mapper.label+'{id:row.id}) on match set n+=row on create set n=row;\r\n')
        
    #for edge definitions
    for mapper in edge_mappers:
        f=open(root+'nbi_'+mapper.rel_id+'.csv',file_mode,newline='')
        writer=csv.writer(f)
        ...
        if len(prop_map)>2:
            script_file.write('using periodic commit 500 LOAD CSV WITH HEADERS FROM "file:///nbi_'+mapper.rel_id+'.csv" AS row match (s:'+mapper.src_label+'{id:row.src_id}),(d:'+mapper.dst_label+'{id:row.dst_id}) merge (s)-[r:'+mapper.label+']->(d) on match set r+='+prop_map+' on create set r='+prop_map+';\r\n')
        else:
            script_file.write('using periodic commit 500 LOAD CSV WITH HEADERS FROM "file:///nbi_'+mapper.rel_id+'.csv" AS row match (s:'+mapper.src_label+'{id:row.src_id}),(d:'+mapper.dst_label+'{id:row.dst_id}) merge (s)-[r:'+mapper.label+']->(d);\r\n')
    
    post_file.write('match (n:Bridge) set n.loc_point=point({longitude:n.longitude,latitude:n.latitude})')
    return writers
